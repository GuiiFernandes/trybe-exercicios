...
public class BusLineService implements BaseService<BusLine> {
    ...
    public List<BusLine> getFutureEntities(LocalDate date) {
        ...
        return busesLines.stream().peek(busLine -> {
    ...
    public List<BusLine> getFutureEntitiesById(LocalDate date, Long id) {
        List<BusLine> busesLines = busLineRepository.findById(id).stream().toList();
        if (busesLines.isEmpty()) {
            return List.of();
        }
        return busesLines.stream().peek(busLine -> {
            List<Schedule> schedules = busLine.getSchedules()
